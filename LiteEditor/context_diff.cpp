...
int ContextDiff::GetHyperlinkRange(int pos, int& start, int& end)
{
    ...
    wxString fileName;
    if(line.StartsWith(wxT("Index: "), &fileName) || line.StartsWith(wxT("--- "), &fileName) ||
       line.StartsWith(wxT("+++ "), &fileName)) {
...
void ContextDiff::GoHyperlink(int start, int end, int type, bool alt)
{
    ...
    if(fn.IsOk()) { clMainFrame::Get()->GetMainBook()->OpenFile(fn.GetFullPath(), wxEmptyString, lineNum); }
}
