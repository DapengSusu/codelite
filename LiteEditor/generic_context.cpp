...
void ContextGeneric::ProcessIdleActions()
{
    ...
    if((ctrl.GetLexerId() == wxSTC_LEX_XML) || (ctrl.GetLexerId() == wxSTC_LEX_PHPSCRIPT) ||
       (ctrl.GetLexerId() == wxSTC_LEX_HTML)) {
        ...
        if(word.IsEmpty()) {
            ctrl.GetCtrl()->SetIndicatorCurrent(MARKER_CONTEXT_WORD_HIGHLIGHT);
            ctrl.GetCtrl()->IndicatorClearRange(0, ctrl.GetLength());
            return;
        }
        ...
        if(reOpenHtmlTag.Matches(word)) {
            ...
            while(where != wxNOT_FOUND) {
                ...
                if((closeTag == word) && (depth == 0)) {
                    ...
                    // Set the new markers
                    ctrl.GetCtrl()->IndicatorFillRange(startPos, endPos - startPos);
                    ctrl.GetCtrl()->IndicatorFillRange(startPos2, endPos2 - startPos2);
                    return;
            ...
            if(!reOpenTag.IsValid()) { clDEBUG() << "Invalid regex:" << reString << clEndl; }
